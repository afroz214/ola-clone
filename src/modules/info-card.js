		...
		<CompactCard removeBottomHeader={"true"}>
			<DivContent>
				<Row>
					...
					<TagLineDiv>
						<HeaderTagLine>
							{`${IcDetails?.tag_line || PlanDetails?.tagline || "N/A"}`}
						</HeaderTagLine>
						<PTagLine>
					...
					<DivSumIns>
						<HeaderSumIns>
							Sum Insured {PerPerson ? "per Member" : <noscript />}
						</HeaderSumIns>
						<PSumIns>
							{`${Currency * 1 === 1 ? "$" : "£"} ${PlanDetails?.sum_insured}`}
						</PSumIns>
					</DivSumIns>
				</Row>
				<Row>
					<DivPremium>
						<HeaderPremium>Premium</HeaderPremium>
					</DivPremium>
				</Row>
				<RowTagPlan xs={1} sm={1} md={1} lg={1} xl={1}>
					<UlTag>
						<LiTag>
							Base Plan Premium
							<SpanTagRight>{`₹ ${
								PlanDetails?.base_premium_amount || "N/A"
							}`}</SpanTagRight>
						</LiTag>
						<LiTag>
							GST
							<SpanTagRight>{`₹ ${
								Math.round(PlanDetails?.tax_amount * 1) || "N/A"
							}`}</SpanTagRight>
						</LiTag>
					</UlTag>
				</RowTagPlan>
				<Row xs={1} sm={1} md={1} lg={1} xl={1}>
					<DivTotal>
						...
						<div>
							<StrongTag>{`₹ ${
								PlanDetails?.total_premium_amount || "N/A"
							}`}</StrongTag>
						</div>
					</DivTotal>
				</Row>
